...
class result_line(object):
    def __init__(self, m_obj, **args):
        self.__v_dict = dict([(key, m_obj.group(key)) for key in ["test",
                                                                  "n",
                                                                  "nb",
                                                                  "p",
                                                                  "q",
...
class hpl_result(object):
    ...
    def _add_result(self, m_obj, **args):
        ...
        if self.__min_result == None:
            self.__min_result, self.__max_result = (act_result,
                                                    act_result)
        else:
...
class hpl_file(hpl_result):
    ...
    def get_info(self):
        num_passed, num_failed = (sum([1 for res in self.iterate_results() if res["passed"]]),
        ...
        min_res, max_res = (self.get_min_result(),
                            self.get_max_result())
        if min_res:
            return "Node %s (%s%s): %3d tests, %s (%s), %6.2f GFlop min (%8s), %6.2f GFlop max (%8s)%s" % (self.node_name,
...
def main():
    try:
        ...
        print "Error parsing commandline %s: %s (%s)" % (" ".join(sys.argv[1:]),
                                                         process_tools.get_except_info())
        sys.exit(1)
    loc_config = configfile.configuration("hpl_parser", {"VERBOSE"          : configfile.int_c_var(0),
                                                         "MERGE_SAME_HOSTS" : configfile.bool_c_var(False)})
    pname = os.path.basename(sys.argv[0])
    verbose, ignore_empty = (0, True)
    for opt, arg in opts:
        if opt in ["-h", "--help"]:
