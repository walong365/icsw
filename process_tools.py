...
class device_recognition(object):
    def __init__(self, dc, **args):
        ...
        dc.execute("SELECT i.ip FROM netip i, netdevice n, device d WHERE n.device=d.device_idx AND i.netdevice=n.netdevice_idx AND d.name='%s'" % (self.short_host_name))
        self.local_ips = [x["ip"] for x in dc.fetchall()]
        self_ips = []
        ...
        if not c_stat:
            for line in [x for x in [y.strip() for y in out.strip().split("\n")] if x.startswith("inet")]:
                ...
                if ip_m and ip_m.group("ip") not in self.local_ips:
...
def is_server(dc, server_type, long_mode=False, report_real_idx=False, short_host_name=""):
    ...
    if dc:
        ...
        # old version
        dc.execute("SELECT d.name, d.device_idx, dc.new_config, c.name AS confname, dc.device FROM device d " + \
        ...
        if num_servers == 1:
            if all_servers[0]["device"] == all_servers[0]["device_idx"]:
                s_type = "real"
            else:
                s_type = "meta"
            server_idx, s_type, s_str, config_idx, real_server_name = (all_servers[0][report_real_idx and "device_idx" or "device"],
                                                                       s_type,
                                                                       "%s '%s'-server via hostname '%s'" % (s_type, server_type, short_host_name),
                                                                       all_servers[0]["new_config"],
                                                                       all_servers[0]["confname"])
        else:
            # get local devices
...
def find_file(file_name, s_path=None):
    if not s_path:
        s_path = ["/bin", "/usr/bin", "/sbin", "/usr/sbin"]
    found = False
