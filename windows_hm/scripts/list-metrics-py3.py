...
def get_perf_data(counters, fmts='long', english=False, delay=0):
    ...
    '''
    if type(counters) is list:  counters = [ c  for c in counters ]
    ...
    if errs:
        raise WindowsError('PdhOpenQueryW failed: {}'.format(get_pd_err(errs)))
    ...
    for counter in counters:
        ...
        if errs:
            raise WindowsError('PdhAddCounterW failed: {}'.format(get_pd_err(errs)))
        hCounters.append(hCounter)

    # Collect
    errs = pdh.PdhCollectQueryData(hQuery)
    if errs:
        raise WindowsError('PdhCollectQueryData failed: {}'.format(get_pd_err(errs)))
    if delay:
    ...
    for i, hCounter in enumerate(hCounters):
        ...
        errs = pdh.PdhGetFormattedCounterValue(hCounter, ifmts[i], None,
                                               byref(value))
        if errs:
            raise WindowsError('PdhGetFormattedCounterValue failed: {}'.format(get_pd_err(errs)))
        values.append(value)

    # Close
    errs = pdh.PdhCloseQuery(hQuery)
    if errs:
        raise WindowsError('PdhCloseQuery failed: {}'.format(get_pd_err(errs)))
...
if __name__=="__main__":    
    ...
    print(' Total: {} b'.format(fmt(meminfo.TotalPhys)))
    print(' usage: {}%'.format(fmt(meminfo.MemoryLoad)))
    print()

    print('Performance Stats:')
    pinfo = get_perf_info()
    print(' Cache: {} p'.format(fmt(pinfo.SystemCache,)))
    print(' Cache: {} b'.format(fmt(pinfo.SystemCacheBytes)))
    print()
    ...
    for drive in drives:
        ...
        try:
            fsinfo = get_fs_usage('{}:\\'.format(drive))
            vinfo = get_vol_info(drive)
            print(' Name: {} {}'.format(vinfo.name, vinfo.serialno))
            print(' Type: {}'.format(vinfo.fstype))
            print(' Total: {}'.format(fmt(fsinfo.total)))
            print(' Used: {}'.format(fmt(fsinfo.used)))
            print(' Free: {}'.format(fmt(fsinfo.free)))
