...
def check_package(image_idx, root_dir, single_package, dc, **args):
    ...
    stat, p_list = subprocess.getstatusoutput(query_str)
    if stat:
                ...
                if pfix == "n":
                    ...
                    if act_p:
                        if pfix in list(rel_dict.keys()):
                            ...
                            print("Error: postfix %s not in dictionary (%s)" % (pfix, ", ".join(list(rel_dict.keys()))))
                        if pfix == "p":
                            num_p += 1
                            # check for new architecture/vendor/distribution
                            for what in ["architecture", "vendor", "distribution"]:
                                if act_p[what] not in glob_dict[what]:
        ...
        if image_idx:
            ...
            if im_info:
                ...
                for p_name in list(packages.keys()):
...
def main():
    ...
    if is_error:
        ...
        if single_package and num_p:
            pack_struct = list(packages.values())[0][0]
