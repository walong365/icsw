...
class job_thread(threading_tools.thread_obj):
    ...
    def _pe_start(self):
        ...
        if not self.__return_value:
            ...
            if self.__opt_dict["CALLER_NAME_SHORT"] == "lamstart":
                self._call_command("lamboot", "%%s -v %s" % (self.__opt_dict["HOSTFILE_ONE_PER_LINE"]), hold_on_error=True)
    def _pe_stop(self):
        ...
        if self.__opt_dict["CALLER_NAME_SHORT"] == "lamstop":
            self._call_command(["lamwipe", "wipe"], "%%s -v %s" % (self.__opt_dict["HOSTFILE_ONE_PER_LINE"]), hold_on_error=False)
        self._kill_foreign_pids(self.__node_list)
        self._remove_foreign_ipcs(self.__node_list)
        self._umount_nfs_mounts(self.__node_list)
    ...
    def _write_hosts_file(self, action):
        ...
        for var_name, file_name, generator in [("HOSTFILE_ONE_PER_LINE", "/tmp/hostfile_opl_%s" % (self.__opt_dict["FULL_JOB_ID"]), self._whf_opl),
    ...
    def _whf_wcpu(self):
        return ["%s cpu=%d" % (node_name, self.__node_dict[node_name]["num"]) for node_name in self.__node_list]
    def _whf_wslot(self):
        return ["%s maxslots=%d" % (node_name, self.__node_dict[node_name]["num"]) for node_name in self.__node_list]
    # job monitor stuff
