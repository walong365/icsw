...
class diff_match_patch:
    ...
    def diff_cleanupSemantic(self, diffs):
        ...
        while pointer < len(diffs):
            if diffs[pointer][0] == self.DIFF_EQUAL:  # Equality found.
                ...
                # sides of it.
                if (lastequality and (len(lastequality) <= max(length_insertions1, length_deletions1)) and
    ...
    def patch_addContext(self, patch, text):
        ...
        while (
            text.find(pattern) != text.rfind(pattern) and (
                self.Match_MaxBits == 0 or len(pattern) < self.Match_MaxBits - self.Patch_Margin - self.Patch_Margin
            )
        ):
    ...
    def patch_make(self, a, b=None, c=None):
        ...
        if isinstance(a, basestring) and isinstance(b, basestring) and c is None:
            ...
            text1 = a
            diffs = b
        elif (isinstance(a, basestring) and isinstance(b, basestring) and isinstance(c, list)):
        ...
        for patch in patches:
            ...
            if start_loc == -1:
                ...
                if text1 == text2:
                    ...
                    if (
                        len(text1) > self.Match_MaxBits and self.diff_levenshtein(diffs) / float(len(text1)) > self.Patch_DeleteThreshold
                    ):
            ...
            while len(bigpatch.diffs) != 0:
                ...
                while (len(bigpatch.diffs) != 0 and patch.length1 < patch_size - self.Patch_Margin):
                    ...
                    if diff_type == self.DIFF_INSERT:
                        ...
                        diff_type == self.DIFF_DELETE and
                        len(patch.diffs) == 1 and
                ...
                if postcontext:
                    ...
                    if len(patch.diffs) != 0 and patch.diffs[-1][0] == self.DIFF_EQUAL:
                        patch.diffs[-1] = (
                            self.DIFF_EQUAL, patch.diffs[-1][1] +
                            postcontext
                        )
                    else:
