...
class netdevice(models.Model):
    ...
    def find_matching_snmp_network_type(self):
        match_list = [ndt for ndt in snmp_network_type.objects.exclude(Q(regex="")) if ndt.match(self.devname)]
        if len(match_list) == 0:
            return None
    ...
    def get_dummy_macaddr(self):
        return ":".join(["00"] * self.snmp_network_type.mac_bytes)
...
def netdevice_pre_save(sender, **kwargs):
    if "instance" in kwargs:
        ...
        _cur_nw_type = cur_inst.snmp_network_type_id
        cur_inst.snmp_network_type = cur_inst.find_matching_snmp_network_type()
        _nw_type_changed = _cur_nw_type != cur_inst.snmp_network_type_id
        # fix None as vlan_id
        ...
        dummy_mac, mac_re_str = (
            ":".join(
                [
                    "00"
                ] * cur_inst.snmp_network_type.mac_bytes
            ),
            "^{}$".format(
                ":".join(
                    [
...
class snmp_network_type(models.Model):
    ...
    def match(self, devname):
        if devname.count(":") == 1:
            ...
            _m_name = devname
        return re.match(self.regex, _m_name)
