...
class ServiceContainer(object):
    ...
    def __init__(self, log_com):
        ...
        self.__django_init = None

    def log(self, what, log_level=logging_tools.LOG_LEVEL_OK):
        self.__log_com(u"[SrvC] {}".format(what), log_level)

    def _check_for_django_init(self):
        if self.__django_init is None:
            os.environ.setdefault("DJANGO_SETTINGS_MODULE", "initat.cluster.settings")
            self.__django_init = False
            try:
    ...
    def check_service(self, opt_ns, entry, use_cache=True, refresh=True):
        ...
        if entry.find("result"):
            if refresh:
                ...
                return
        dev_config = []
        if self.__django_init and self.config_tools and entry.find(".//config_names/config_name") is not None:
            ...
            for _conf_name in _conf_names:
                _cr = self.config_tools.server_check(server_type=_conf_name)
