...
# -*- encoding: utf-8 -*-
#
# Copyright (c) 2001-2004,2007-2009,2013-2017 Andreas Lang-Nevyjel, init.at
#
...
class Parser(object):
    ...
    def _add_user_parser(self, sub_parser, server_mode):
        ...
        if server_mode:
            ...
            parser.add_argument("--show-vars", default=False, action="store_true", help="show user variables when mode=='info' [%(default)s]")
            parser.add_argument(
                "--delete-var",
                default=0,
                type=int,
                help="variable to delete [index, %(default)s]",
            )
            if hel.exp_dict:
                # build dict
                parser.add_argument(
