# Copyright (C) 2007,2013-2017 Lang-Nevyjel
#
...
class get_file_content(cs_base_class.icswCSServerCom):
    def _call(self, cur_inst):
        filesys_tools.get_file_content(cur_inst.srv_com, cur_inst.log)


class set_file_content(cs_base_class.icswCSServerCom):
    def _call(self, cur_inst):
        filesys_tools.set_file_content(cur_inst.srv_com, cur_inst.log)


class get_dir_tree(cs_base_class.icswCSServerCom):
    def _call(self, cur_inst):
        filesys_tools.get_dir_tree(cur_inst.srv_com, cur_inst.log)


class create_dir(cs_base_class.icswCSServerCom):
    def _call(self, cur_inst):
        filesys_tools.create_dir(cur_inst.srv_com, cur_inst.log)


class remove_dir(cs_base_class.icswCSServerCom):
    def _call(self, cur_inst):
