...
def check_for_association(dc, p_idx, name, version, release, set_dict):
    ...
    if p_idx in pip_dict:
        for del_idx in list(pip_dict[p_idx].keys()):
            if set_dict:
                if len([1 for sf in [pip_dict[p_idx][del_idx][key] == set_dict[key] for key in list(set_dict.keys()) if key not in ["last_build"]] if not sf]) > 0:
                    is_same = False
                else:
                    is_same = True
            else:
...
def main():
    ...
    if dc:
        p_idx = None
        stat, out = subprocess.getstatusoutput("/usr/local/sbin/insert_package_info.py -p %s" % (long_package_name))
        if stat:
        ...
        if dc:
            ...
            if write_entry:
                sql_str, sql_tuple = ("INSERT INTO inst_package SET %s" % (", ".join(["%s=%%s" % (key) for key in list(set_dict.keys())])),
