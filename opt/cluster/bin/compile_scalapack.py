...
class my_opt_parser(argparse.ArgumentParser):
    ...
    def _read_versions(self):
        if os.path.isfile(self.VERSION_FILE):
            version_lines = [
                line.strip().split() for line in open(self.VERSION_FILE, "r").read().split("\n") if line.strip()
            ]
            self.version_dict = {
                key: value for key, value in version_lines
            }
            vers_dict = {
                tuple([part.isdigit() and int(part) or part for part in key.split(".")]): key for key in list(self.version_dict.keys())
            }
