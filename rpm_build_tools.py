...
class package_parser(argparse.ArgumentParser):
    def __init__(self):
        ...
        _arch = {"i686" : "i586"}.get(os.uname()[4], os.uname()[4])
        self.add_argument("-s", "--summary", dest="summary", help="Summary string [%(default)s]", default="files and packages for directory")
        self.add_argument("-d", "--description", dest="description", help="Description string [%(default)s]", default="autogenerated package")
        self.add_argument("-p", "--packager", dest="packager", help="set packager [%(default)s]", default="%s@%s" % (pwd.getpwuid(os.getuid())[0], process_tools.get_machine_name()))
        self.add_argument("-P", "--package-group", dest="package_group", help="set package-group [%(default)s]", default="System/Monitoring")
        self.add_argument("-u", "--user", dest="user", default="root", help="user to use for package [%(default)s]")
        self.add_argument("-g", "--group", dest="group", default="root", help="group to use for package [%(default)s]")
        self.add_argument("-n", dest="name", default="", help="name of package [%(default)s]", required=True)
        self.add_argument("-v", dest="version", default="1", help="package version [%(default)s]")
        self.add_argument("-r", dest="release", default="0", help="package release [%(default)s]")
        self.add_argument("-a", "--arch", dest="arch", help="set package architecture [%(default)s]", default=_arch)
        self.add_argument("--post-script", default="", help="filename of post-script content")
