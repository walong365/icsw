        ...
        for hosts in all_hosts:
            for host in hosts:
                ...
                if not (host["alias"].strip() and host["alias_excl"]):
                    host_names.append("%s%s" % (host["name"], host["postfix"]))
                ...
                if host["short_names"]:
                    ...
                    out_names = (" ".join(["%s.%s %s" % (host_name, host["domain_name"], host_name) for host_name in host_names if not host_name.count(".")])).split()
                else:
                    # only print the long names
                    out_names = ["%s.%s" % (host_name, host["domain_name"]) for host_name in host_names if not host_name.count(".")]
                # add names with dot
                out_names.extend([host_name for host_name in host_names if host_name.count(".")])
                # name_dict without localhost
                name_dict.setdefault(host["name"], []).extend([out_name for out_name in out_names if out_name not in name_dict[host["name"]] and not out_name.startswith("localhost")])
                ip_dict.setdefault(host["ip"], [])
                if out_names not in [x[1] for x in ip_dict[host["ip"]]]:
                    ip_dict[host["ip"]].append((host["value"], out_names))
        # out_list
        loc_dict = {}
