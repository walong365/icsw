...
int main (int argc, char** argv) {
    int mysock, ret, num, inlen, file, i/*, time*/, port, rchar, verbose, quiet, retcode, write_file, timeout;
...
/*  errno = 0;*/
    ...
    *act_pos = 0;
    ...
    if ((ret = sigaction(SIGALRM, (struct sigaction*)alrmsigact, NULL))<0) {
        ...
        ret = connect(mysock, (struct sockaddr*) sockaddr, sizeof(struct sockaddr));
        if (ret<0) {
        ...
        inlen = atoi(iobuff);
        if (num != 8) {
            syslog(LOG_DAEMON|LOG_ERR, "short read while receiving response header for '%s' to %s (got only %d instead of 8 bytes), skipping further reads\n", sendbuff, (char*)host_b, num);
            ...
            if (! num) {
                syslog(LOG_DAEMON|LOG_ERR, "empty read while receiving response body for '%s' to %s\n", sendbuff, (char*)host_b);
            } else if (num != inlen) {
                syslog(LOG_DAEMON|LOG_INFO, "short read while receiving response body for '%s' to %s (got only %d instead of %d bytes)\n", sendbuff, (char*)host_b, num, inlen);
                inlen -= num;
            } else {
                act_bp[num] = 0;
                syslog(LOG_DAEMON|LOG_INFO, "successfully sent '%s' to host %s, port %d, received %d bytes\n", sendbuff, host_b, port, num);
                retcode = STATE_OK;
                if (!quiet) {
                printf("%s\n", iobuff);
                }
