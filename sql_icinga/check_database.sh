...
cdir=/opt/cluster/md_daemon/sql_icinga
...
ICINGA_DATABASE=nagiosdb
TEMP_DATABASE=ngtemp

echo "Deleting old tables and creating temporary icinga database ..."
echo "DROP DATABASE IF EXISTS $TEMP_DATABASE ; CREATE DATABASE $TEMP_DATABASE ; " | mysql -u ${MYSQL_USER} -h ${MYSQL_HOST} -p${MYSQL_PASSWD} 

echo "Creating icinga tables ..."
mysql -u ${MYSQL_USER} -h ${MYSQL_HOST} -p${MYSQL_PASSWD} ${TEMP_DATABASE} < ${cdir}/mysql.sql

dump_orig=`mktemp /tmp/mysql_dump_XXXXXX`
dump_temp=`mktemp /tmp/mysql_dump_XXXXXX`
# original database
mysqldump -u ${MYSQL_USER} -h ${MYSQL_HOST} -p${MYSQL_PASSWD} ${ICINGA_DATABASE} --add-drop-table -d | sed -r s/\ AUTO_INCREMENT=[0-9]+//g > $dump_orig
...
if [ "$verify" = "1" ] ; then
    /usr/local/cluster/bin/check_database.py --verify ${ICINGA_DATABASE} ${TEMP_DATABASE}
else
    /usr/local/cluster/bin/check_database.py ${ICINGA_DATABASE} ${TEMP_DATABASE}
fi
