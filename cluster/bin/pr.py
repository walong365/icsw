...
MOD_REFUSE_LIST = ["3w-9xxx", "3w-xxxx", "af_packet", "ata_piix",
                   "autofs", "eata", "ext3", "gdth", "runrpc",
                   "jbd", "libata", "lockd", "mptbase",
                   "mptctl", "mptscsih", "nfs", "reiserfs",
                   "sunrpc", "sata_promise", "sata_via", "scsi_mod",
                   "scsi_mod", "sd_mod", "sg", "sym53c8xx", "unix"]

LINUXRC_NAME = "init"
# block size for command_stack
CS_BLOCK_SIZE = 8000
...
stage1_file_dict = {0 : ["inetd", "xinetd", "in.rshd", "tcpd", "in.rlogind", "whoami", "ntpdate", "ps", "rmmod", "rmmod.old", "lsmod.old", "depmod.old",
                         "insmod.old", "modprobe.old", "route", "free", "arp", "login"],
                    1 : ["ethtool", "ash", "cp", "mount", "cat", "ls", "mount", "mkdir",
                         "tar", "gunzip", "umount", "rmdir", "egrep", "fgrep", "grep", "rm", "chmod",
                         "sed", "dmesg", "ping", "mknod", "true", "false", "logger", "modprobe",
...
stage2_file_dict = {0 : ["inetd", "xinetd", "mkfs.xfs", "rmmod.old", "lsmod.old", "depmod.old", "insmod.old",
                         "modprobe.old", "in.rshd", "in.rlogind"],
                    1 : ["ethtool", "sh", "strace", "bash", "echo", "cp", "mount", "cat", "ls", "mount", "mkdir",
                         ...
                         "mkfs.ext3", "fdisk", "cfdisk", "sfdisk", "parted", "ifconfig", "mkfs.reiserfs", "mkswap",
                         "reboot", "halt", "shutdown", "init", "route", "tell_mother", "lilo", "grub-install", "grub", "date", "tune2fs",
                         ["syslogd", "syslog-ng"], "bzip2", "bunzip2", "cut", "tr", "chroot", "whoami", "killall", "head", "tail",
                         "seq", "tcpd", "hoststatus", "ldconfig", "sort", "dirname", "vi", "hostname",
                         "chown", "wc", "portmap", "klogd", "arp", "ln", "find", "tftp", "uname", "rsync", "stty", "reset", "id", "lspci"]}
...
def find_module(md, dir, files):
    ...
    for f_name in files:
        act_f = "%s/%s" % (dir, f_name)
        if os.path.isfile(act_f):
            if f_name in mod_list:
                md[f_name.split(".")[0]] = act_f
...
def get_module_dependencies(kern_dir, mod_list):
    ...
    for mod_name in mod_list:
        mod_dict[mod_name.split(".")[0]] = None
    ...
    if os.path.isfile(dep_file):
        ...
        auto_dep = []
    not_found_mods = [k for k, v in mod_dict.iteritems() if not v]
    found_mods = [v for k, v in mod_dict.iteritems() if v]
    return found_mods + auto_dep, not_found_mods
...
def populate_it(stage_num, temp_dir, in_dir_dict, in_file_dict, ignore_errors, show_content):
    ...
    if major < 2 and minor < 3:
        print "Need at least pyton 2.3.x, exiting..."
        sys.exit(-1)
    file_list = []
    ...
    for file_name in new_files:
        ...
        if os.path.islink(file_name):
            ...
            if stat.S_ISCHR(file_stat.st_mode):
                ...
                # character device
                if minor < 3:
                    # code to emulate functions missing 2.2.x 
                    f_major = commands.getoutput("ls -l %s | tr -s ' ' | cut -d ' ' -f 5 | cut -d ',' -f 1" % (file_name))
                    f_minor = commands.getoutput("ls -l %s | tr -s ' ' | cut -d ' ' -f 6 | cut -d ',' -f 1" % (file_name))
    ...
    if stage_num == 1:
        for name in "sh", "bash":
            src_name, dst_name = ("ash", "%s/bin/%s" % (temp_dir, name))
            if verbose > 1:
...
def main_local():
    ...
    script = sys.argv[0]
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hs:d:ilv", ["help"])
    except getopt.GetoptError, why:
        print "Error parsing commandline : %s" % (str(why))
        sys.exit(1)
    stage_num, verbose, temp_dir, ignore_errors, show_content = (1, 0, None, 0, 0)
    for opt, arg in opts:
        if opt in ["-h", "--help"]:
    ...
    if stage_num == 1:
        stage_ok = populate_it(stage_num, temp_dir, stage1_dir_dict, stage1_file_dict, ignore_errors, show_content)
    else:
        stage_ok = populate_it(stage_num, temp_dir, stage2_dir_dict, stage2_file_dict, ignore_errors, show_content)
    sys.exit(stage_ok)
...
def do_show_kernels(dev_assoc):
    ...
    if dev_assoc:
        ...
        dbcon.dc.execute("SELECT d.name AS devname, k.name AS kernname, n.devname AS netname, n.driver FROM device d, kernel k, netdevice n WHERE d.new_kernel=k.kernel_idx AND n.device=d.device_idx AND n.netdevice_idx=d.bootnetdevice ORDER by d.name")
        for db_rec in dbcon.dc.fetchall():
            kern_dict.setdefault(db_rec["kernname"], {}).setdefault(db_rec["driver"], []).append(("new", db_rec["devname"], db_rec["netname"]))
        dbcon.dc.execute("SELECT d.name AS devname, k.name AS kernname, n.devname AS netname, n.driver FROM device d, kernel k, netdevice n WHERE d.newkernel=k.name AND d.new_kernel != k.kernel_idx AND n.device=d.device_idx AND n.netdevice_idx=d.bootnetdevice ORDER by d.name")
        for db_rec in dbcon.dc.fetchall():
        ...
        dbcon.dc.execute("SELECT k.* FROM kernel k ORDER BY k.name")
        for db_rec in dbcon.dc.fetchall():
...
def main_normal():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hs:6m:M:r:vkilDS:T:LqfFt", ["help", "cpio", "cramfs"])
    except getopt.GetoptError, why:
        print "Error parsing commandline : %s" % (str(why))
        sys.exit(1)
    loc_config = configfile.configuration("local_config", {"VERBOSE"             : configfile.int_c_var(0),
                                                           ...
                                                           "SUPRESS_TRANSFER"    : configfile.bool_c_var(False),
                                                           ...
                                                           "DO_CRAMFS"           : configfile.bool_c_var(False),
                                                           "DO_CPIO"             : configfile.bool_c_var(False),
    ...
    for opt, arg in opts:
        if opt in ["-h", "--help"]:
            ...
            print " --cramfs        generate cramfs for stage1"
            print " --cpio          generate cpio for stage1"
            ...
            print " -t              supress transfer to mother_servers"
            sys.exit(0)
        if opt == "-t":
            loc_config["SUPRESS_TRANSFER"] = True
        if opt == "-D":
        ...
        if opt == "-M":
            mods_file = arg
        if opt == "--cramfs":
            loc_config["DO_CRAMFS"] = True
            loc_config["DO_CPIO"] = False
        if opt == "--cpio":
            loc_config["DO_CPIO"] = True
            loc_config["DO_CRAMFS"] = False
        if opt == "-i":
    ...
    if mysql_tools:
        ...
        dbcon = None
    if dbcon:
        is_server, server_idx = process_tools.is_server(dbcon.dc, "kernel_server")
        if not is_server:
            ...
            dbcon.dc.execute("SELECT d.name, d.device_idx FROM device d INNER JOIN device_config dc INNER JOIN new_config c INNER JOIN device_group dg LEFT JOIN device d2 ON d2.device_idx = dg.device WHERE d.device_group=dg.device_group_idx AND dc.new_config=c.new_config_idx AND (dc.device=d.device_idx OR dc.device=d2.device_idx) AND c.name='mother_server'")
            mother_servers = [x["name"] for x in dbcon.dc.fetchall()]
            # get local netdevices
            dbcon.dc.execute("SELECT n.netdevice_idx FROM netdevice n WHERE n.device=%d" % (server_idx))
            all_ids = [x["netdevice_idx"] for x in dbcon.dc.fetchall()]
            dbcon.dc.execute("SELECT d.name, i.ip, h.value, i.alias FROM netip i, hopcount h, netdevice n, device d WHERE i.netdevice=n.netdevice_idx AND n.netdevice_idx=h.d_netdevice AND n.device=d.device_idx AND (%s) AND (%s) ORDER BY h.value, d.name" % (" OR ".join(["d.name='%s'" % (x) for x in mother_servers]),
    ...
    if dbcon:
        ...
        dbcon.dc.execute("SELECT * FROM kernel k WHERE k.name=%s", kernel_name)
        if dbcon.dc.rowcount:
            kernel_stuff = dbcon.dc.fetchone()
    ...
    act_mods = []
    take_modules_from_db = (mods == None and not mods_file)
    if not take_modules_from_db:
        if mods_file:
            if os.path.isfile(mods_file):
                act_mods.extend([y for y in [x.strip() for x in file(mods_file, "r").read().split("\n")] if y])
            else:
    ...
    for stage in range(1, 4):
        fname = "/%s/stage%d" % (loc_config["STAGE_SOURCE_DIR"], stage)
        if not os.path.isfile(fname):
            print "Cannot find stage %d file %s in stage_dir %s" % (stage, fname, loc_config["STAGE_SOURCE_DIR"])
            sys.exit(1)
    if not (loc_config["DO_CRAMFS"] or loc_config["DO_CPIO"]):
    ...
    if not loc_config["QUIET_MODE"]:
        ...
        if loc_config["DO_CRAMFS"]:
            print "  will use mkfs.cramfs to build stage1 initrd"
        elif loc_config["DO_CPIO"]:
            print "  will create a cpio-archive for stage1 initrd"
        else:
    ...
    if kernel_idx:
        if take_modules_from_db:
            ...
            print "Saving module_list to database: %s, %s" % (logging_tools.get_plural("module", len(act_mods)),
                                                              ", ".join(act_mods))
            dbcon.dc.execute("UPDATE kernel SET target_module_list='%s' WHERE kernel_idx=%d" % (",".join(act_mods),
                                                                                                kernel_idx))
    if act_mods:
        all_mods, del_mods = get_module_dependencies(loc_config["KERNEL_DIR"], act_mods)
    ...
    if kernel_idx:
        ...
        dbcon.dc.execute("UPDATE kernel SET module_list='%s', initrd_built=NOW() WHERE kernel_idx=%d" % (",".join(base_mods),
                                                                                                         kernel_idx))
        print "ok"
    ...
    stage1_file = os.path.normpath("/%s/initrd" % (loc_config["STAGE_DEST_DIR"] and loc_config["STAGE_DEST_DIR"] or loc_config["KERNEL_DIR"]))
    ...
    if minor < 3:
        # code to emulate functions missing 2.2.x 
        tempfile.tempdir = "/%s/tmp/" % (loc_config["ROOT_DIR"])
    ...
    stat_out = []
    if loc_config["DO_CRAMFS"] or loc_config["DO_CPIO"]:
        pass
    else:
        stat_out += [("dd"       , commands.getstatusoutput("dd if=/dev/zero of=%s bs=1024 count=%d" % (stage1_file, loc_config["INIT_SIZE"])))]
        stat_out += [("losetup"  , commands.getstatusoutput("losetup %s %s" % (loop_dev, stage1_file)))]
        stat_out += [("mkfs.ext2", commands.getstatusoutput("mkfs.ext2 -F -v -m 0 -b 1024 %s %d" % (stage1_file, loc_config["INIT_SIZE"])))]
    ...
    stat_out = []
    if loc_config["DO_CRAMFS"]:
        stat_out += [("mkfs.cramfs", commands.getstatusoutput("mkfs.cramfs -n initrd_stage1 %s %s" % (stage1_dir, stage1_file)))]
    elif loc_config["DO_CPIO"]:
        act_dir = os.getcwd()
        stat_out += [("cpio", commands.getstatusoutput("cd %s ; find %s -printf \"%%P\\n\" | cpio -c -o > %s ; cd %s" % (stage1_dir, stage1_dir, stage1_file, act_dir)))]
    else:
        stat_out += [("umount" , commands.getstatusoutput("umount %s" % (stage1_file))),
    ...
    if [x for name, (x, y) in stat_out if x]:
        ...
        for name, (err_name, y) in stat_out:
            print "%s (%d) : \n%s" % (name, err_name, "\n".join([" - %s" % (z) for z in y.split("\n")]))
    if stage_dirs_ok:
        print "Compressing stage1 ... ",
        s_time = time.time()
        o_s1_size = os.stat(stage1_file)[stat.ST_SIZE]
        # zip stage1
        gzip.GzipFile("%s.gz" % (stage1_file), "wb", 1).write(file(stage1_file, "rb").read())
        os.unlink(stage1_file)
        stage1_file = "%s.gz" % (stage1_file)
        n_s1_size = os.stat(stage1_file)[stat.ST_SIZE]
        e_time = time.time()
        print "from %s to %s in %5.2f seconds" % (get_size_str(o_s1_size),
        ...
        e_time = time.time()
        print "from %s to %s in %5.2f seconds" % (get_size_str(o_s2_size),
