...
class my_opt_parser(optparse.OptionParser):
    ...
    def get_compile_options(self):
        ...
        ret_lines.extend([" - small_verison is %s" % (self.small_version),
                          " - source package is %s, target directory is %s" % (self.version_dict[self.options.hpl_version],
                                                                               self.hpl_dir),
                          " - package name choosen ist %s" % (self.package_name),
...
class hpl_builder(object):
    ...
    def _generate_makefile(self):
        ...
        if self.parser.options.use_mkl:
            make_list.extend([("LAdir", "%s/lib/intel64" % (self.parser.options.mkl_lib)),
                              ("LAinc", ""),
                              ("LAlib", "-L$(LAdir) $(LAdir)/libmkl_intel_lp64.a $(LAdir)/libmkl_sequential.a $(LAdir)/libmkl_core.a")])
        else:
            make_list.extend([("LAdir", os.path.dirname(self.parser.options.goto_lib)),
                              ("LAinc", ""),
                              ("LAlib", self.parser.options.goto_lib)])
        make_list.extend([("F2CDEFS"     , ""),
    ...
    def package_it(self):
        ...
        if not os.path.isfile(xhpl_file_name):
            ...
            new_p["package_group"] = "Tools/Benchmark"
