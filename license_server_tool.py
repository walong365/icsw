...
class license_object(object):
    ...
    def _read_license(self):
        ...
        for lic_line in self.__lic_lines:
            ...
            if lic_line.startswith("#") or not lic_line:
                ...
                raise ValueError, "unparsable line %d in license-file: %s" % (line_num,
                                                                              lic_line)
    def _check_for_settings(self):
        ...
        if self["ENABLED"] == "yes":
            ...
            found_keys = [x for x in self.__val_dict.keys() if x in needed_keys]
            if len(found_keys) != len(needed_keys):
                raise KeyError, "some keys missing: %s" % (", ".join([x for x in needed_keys if x not in found_keys]))
            if not os.path.isfile(self["LMGRD_BINARY"]):
                raise IOError, "cannot find LMGRD_BINARY %s" % (self["LMGRD_BINARY"])
            if not os.path.isfile(self["LICENSE_FILE"]) and not os.path.isdir(self["LICENSE_FILE"]):
                raise IOError, "cannot find LICENSE_FILE %s (file or directory)" % (self["LICENSE_FILE"])
            self["DEBUG_LOG"] = "%s/%s.dbg" % (LICENSE_LOG_DIR,
                                               self["SAVE_NAME"])
            self["STDOUT_LOG"] = "%s/%s.log" % (LICENSE_LOG_DIR,
                                                self["SAVE_NAME"])
            self["START_SCRIPT"] = "%s/%s.start.sh" % (LICENSE_SCRIPT_DIR,
                                                       self["SAVE_NAME"])
            self["STOP_SCRIPT"] = "%s/%s.stop.sh" % (LICENSE_SCRIPT_DIR,
                                                     self["SAVE_NAME"])
            self._write_scripts()
    ...
    def start_server(self):
        if self.get_pid():
            ...
            if self["USE_ROOT"] == "no":
                start_com = "su - -s /bin/bash %s %s" % (self.__target_user,
                                                         self["START_SCRIPT"])
            else:
    ...
    def _read_licenses(self):
        for lic_file in os.listdir(self.__cfg_dir):
            ...
            try:
                ...
                self.log("Error reading license from file %s: %s" % (full_name,
                                                                     process_tools.get_except_info()),
                         logging_tools.LOG_LEVEL_ERROR)
            else:
                self.log("read license '%s' from file %s" % (new_lic["NAME"],
                                                             full_name))
                self.__num_lics += 1
