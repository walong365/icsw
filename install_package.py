...
def main():
    ...
    if not signal:
        ...
        for targ_str, targ_stuff in out_list.items():
            for flag_str, dev_list in targ_stuff.items():
    ...
    dc.release()
    print("Signaling package-server at '%s' about new config, timeout set to %d seconds ..." % (package_server, act_to), end=' ')
    s_time = time.time()
