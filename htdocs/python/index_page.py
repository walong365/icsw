...
def process_page(req):
    ...
    if req.session_data:
        ...
        if os.path.isfile(direct_link_name):
            ...
            dlink_map = []
        for tl_idx in req.cap_stack.get_sorted_idxs(0, only_authorized=True):
            sub_idxs = req.cap_stack.get_sorted_idxs(tl_idx, only_authorized=True)
            if sub_idxs and len([True for sub_idx in sub_idxs if req.cap_stack[sub_idx].left_string]):
                cg_stuff = req.cap_stack[tl_idx]
                out_table[0]["class"] = "title"
                out_table[None][1:2] = html_tools.content("%s%s" % (cg_stuff.description,
                                                                    SHOW_INDEX_PRI == 1 and " (pri %d)" % (cg_stuff.priority) or ""),
                                                          cls="center")
                for sub_idx in sub_idxs:
                    grp_stuff = req.cap_stack[sub_idx]
                    if grp_stuff.left_string:
                        ...
                        out_table[0][0] = html_tools.content("<a href=\"%s?%s\">%s%s</a>%s" % (act_target,
                                                                                               ...
                                                                                               mapped_str))
                        out_table[None][0] = html_tools.content("(%s) %s" % (grp_stuff.authorized_by, grp_stuff.right_string))
        if dlink_map:
            out_table[0]["class"] = "title"
        ...
        req.session_data.set_php_update(True)
    else:
        header_table[1:2][1] = html_tools.content("<a class=\"init\" href=\"http://www.init.at\" target=\"_top\"><img alt=\"Init.at logo\" src=\"/icons-init/kopflogo.png\" border=0></a>", cls="centersmall")
        header_table[1][2] = html_tools.content(cluster_name, cls="centerlarge")
        ...
        if req.dc.rowcount:
            ...
            users_defined = True
            req.dc.execute("SELECT u.login FROM session_data s, user u WHERE s.remote_addr='%s' AND u.user_idx=s.user_idx ORDER BY s.date DESC LIMIT 1" % (req.environ["REMOTE_ADDR"]))
            if req.dc.rowcount:
                last_user = req.dc.fetchone()["login"]
            else:
                last_user = ""
        else:
