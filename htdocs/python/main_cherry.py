...
def read_standard_config(req):
    ...
    full_server_name = req.server.server_hostname
    ...
    if req.session_data:
        ...
        if req.session_data.has_property("server_routes"):
            ...
            if req.dc.rowcount:
                ...
                if req.user_info and req.user_info.capability_ok("sql"):
                    if req.dc.rowcount:
                        req.info_stack.add_error("Cannot rebuild server_routes (%s): failure: cannot determine webfrontend-host (check /etc/http*/httpd.conf, name found there is '%s')" % (when,
...
def rescan_files(req):
    ...
    dir_name = os.path.dirname(req.filename)
...
def make_basic_entries(req, def_admin, def_passwd):
    ...
    # SNMP classes
    req.dc.execute("INSERT INTO snmp_class SET name='default_class', descr='Standard Class for SNMP-devices'")
    # device classes
...
def handle_normal_module_call(req, module_name):
    ...
    if mysql_tools:
        ...
        #dbcon = mysql_tools.db_con(log_instance=logging_tools.log_instance(log_handler=log_handler, err_handler=err_handler, add_data=req))
    else:
        req.write("<br>Neither MySQLdb nor mysql_tools found<br>")
        return apache.OK
    try:
