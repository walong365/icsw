...
def process_page(req):
    ...
    req.headers_out["Content-Disposition"] = "filename=\"syslog\""
    req.send_http_header()
    log_name = req.sys_args.get("log", None)
    if log_name:
        if log_name.endswith(".gz"):
            req.write(gzip.open(log_name, "r").read())
        else:
            req.write(file(log_name, "r").read())
    else:
        req.write("logfilename not specified")
