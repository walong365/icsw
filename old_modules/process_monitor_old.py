...
class procstat_command(hm_classes.hmb_command):
    def __init__(self, **args):
        hm_classes.hmb_command.__init__(self, "procstat", **args)
        self.help_str = "checks the number of running process for a given process or returns the number of all running processes"
        self.short_client_info = "NAME or nothing -w NUM1 -c NUM2 -Z"
        self.long_client_info = "if NAME is omitted all processes are returned, -Z to reduce severity of zombies"
        self.short_client_opts = "w:c:Z"
        self.net_only = True
...
class proclist_command(hm_classes.hmb_command):
    def __init__(self, **args):
        hm_classes.hmb_command.__init__(self, "proclist", **args)
