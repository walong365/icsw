...
$XML_PATH/xml ed -u "//key[@name='log.mail.from.name']" -v 'VAGRANT_logging-server' -u "//key[@name='mail.target.address']" -v 'bahlon@init.at' -u "//key[@name='meta.mail.from.name']" -v 'VAGRANT_meta-server' $CONF > $TMP
